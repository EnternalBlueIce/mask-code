...
def construct_knn(kernel_idx, jug):
    edge_index = [[], []]#初始化边集，edge_index[0]代表源节点集，edge[1]代表目标节点集

    for i in range(len(kernel_idx)):
        l = len(kernel_idx[i])
        if jug[i] and l > 0:#如果这张图是大图，就减少一个邻居，对应非公平连接
            l = l - 1
        ...
        for j in range(l):
            ...
            edge_index[0].append(i)#在i和j之间创建一条无向边
...
def get_kernel_knn(dataname, kernel_type, knn_nei_num, jug):
    ...
    if(os.path.exists(kernel_file)):
        ...
        G = dataset.data#获取所有图集合
        if(dataname in ['IMDB-BINARY', 'REDDIT-BINARY']):
            gk = ShortestPath(normalize=True, with_labels=False)
        else:
            gk = ShortestPath(normalize=True)#创建最短路径核
        kernel_simi = torch.tensor(gk.fit_transform(G))#计算图相似性矩阵
        torch.save(kernel_simi, kernel_file)
    kernel_idx = torch.topk(kernel_simi, k=knn_nei_num,
                            dim=1, largest=True)[1][:, 1:]#得到最相似的前k张图的索引
    knn_edge_index = construct_knn(kernel_idx, jug)#构建KNN图，返回边的索引，（源节点，目标节点）的形式
    return kernel_idx, knn_edge_index#返回K个邻居的索引
