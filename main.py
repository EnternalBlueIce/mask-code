...
if __name__ == '__main__':
    args = parse_args() #读取超参数
    torch.cuda.set_device(args.gpu)#绑定指定GPU
    args.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')#如果没有GPU就用CPU
    args.path = os.getcwd()
    
    dataset, args.n_feat, args.n_class, _ = get_TUDataset(
        args.dataset, pre_transform=T.ToSparseTensor())#加载图数据集，得到图集合，图的特征维度，类数量

    args.c_train_num, args.c_val_num = get_class_num(
        args.imb_ratio, args.num_train, args.num_val)#根据不平衡率，生成训练集、验证集的样本数量

    args.y = torch.tensor([data.y.item() for data in dataset])#提取标签，得到一个标签数组
    size_lst = [data.x.shape[0] for data in dataset]#获取节点数
    args.size = torch.tensor(size_lst)
    size_lst.sort()
    args.mid = size_lst[int(args.size_ratio * len(size_lst))]
    args.sizeb = args.size >= args.mid#标记大图
    ...
    for data in dataset:
        ...
        row, col = edge_index
        deg = degree(col)
        args.degs.append(deg.mean().item())#计算节点度的平均值
    degs = sorted(args.degs)
    args.middeg = degs[int(args.deg_ratio * len(degs))]#得到度阈值
    args.degs = torch.tensor(args.degs)
    args.degb = args.degs > args.middeg#找到大于度阈值的图
    scal = np.log(degs[-1] / degs[0])
    args.p_edge = (args.aug_ratio - (1 - args.aug_ratio)) * torch.log(args.degs / degs[0]) / scal + 1 - args.aug_ratio#对边作出增强的概率
    args.p_node = ((1 - args.aug_ratio) - args.aug_ratio) * torch.log(args.degs / degs[0]) / scal + args.aug_ratio#对点作出增强的概率

    args.kernel_idx, args.knn_edge_index = get_kernel_knn(args.dataset, args.kernel_type, args.knn_nei_num, args.sizeb)#构建并获取KNN图

    F1_micro, F1_macro = run(args)#执行训练和测试

    print('F1_macro: ', np.mean(F1_macro))
    print('F1_micro: ', np.mean(F1_micro))#得到最终结果
